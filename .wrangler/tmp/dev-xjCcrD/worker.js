...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      if (request.method === "POST" && (url.pathname === "/api/waitlist" || url.pathname === "/v1/api/waitlist")) {
      ...
      if (request.method === "GET" && (url.pathname.startsWith("/app/pages/") || url.pathname.startsWith("/v1/app/pages/"))) {
        const slug = decodeLastSegment(url.pathname);
        return handleGetPublicPageBySlug(env, corsHeaders, slug);
      }
      if (request.method === "GET" && (url.pathname.startsWith("/app/content/") || url.pathname.startsWith("/v1/app/content/"))) {
        const slug = decodeLastSegment(url.pathname);
...
function stripV1Prefix(path) {
  return path.startsWith("/v1/") ? path.slice(3) : path;
}
__name(stripV1Prefix, "stripV1Prefix");
function decodeLastSegment(path) {
  const parts = path.split("/").filter(Boolean);
  return decodeURIComponent(parts[parts.length - 1] ?? "");
}
__name(decodeLastSegment, "decodeLastSegment");
...
async function handleWaitlistSignup(request, env, corsHeaders) {
  ...
  const inserted = await supabase.from("waitlist").insert({ email, name: name || null, meta: metadata }).select("*").single();
  if (inserted.error) {
...
async function handleOpsLogin(request, env, corsHeaders) {
  const body = await safeParseJson(request);
  const email = typeof body?.email === "string" ? body.email.trim().toLowerCase() : "";
  ...
  if (isFallbackMatch) {
    const adminId2 = crypto.randomUUID();
    const roleName2 = "super_admin";
    const token2 = await signToken({ sub: adminId2, email, role_name: roleName2, exp: Math.floor(Date.now() / 1e3) + 43200 }, env);
    return jsonResponse({ token: token2, admin: { id: adminId2, email, role_name: roleName2 } }, 200, corsHeaders);
  }
  const supabase = getSupabase(env);
  ...
  const token = await signToken({ sub: adminId, email, role_name: roleName, exp: Math.floor(Date.now() / 1e3) + 43200 }, env);
  if (admin) {
    await supabase.from("ops_admins").update({ last_login: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", admin.id);
  }
...
__name(handleOpsLogin, "handleOpsLogin");
async function requireOpsAuth(request, env) {
...
__name(requireOpsAuth, "requireOpsAuth");
async function handleGetPublicConfig(env, corsHeaders) {
...
async function handleGetOpsBranding(env, corsHeaders) {
  const row = await getOrCreateConfigRow(env);
  return jsonResponse(row, 200, corsHeaders);
}
__name(handleGetOpsBranding, "handleGetOpsBranding");
async function handlePutOpsBranding(request, env, corsHeaders) {
  const body = await safeParseJson(request) ?? {};
  const supabase = getSupabase(env);
  const payload = {
...
__name(handlePutOpsBranding, "handlePutOpsBranding");
async function handleGetPublicContact(env, corsHeaders) {
  const row = await getOrCreateContactRow(env);
  return jsonResponse(row, 200, corsHeaders);
}
__name(handleGetPublicContact, "handleGetPublicContact");
async function handlePostContactMessage(request, env, corsHeaders) {
  const body = await safeParseJson(request);
...
__name(handlePostContactMessage, "handlePostContactMessage");
async function handleGetOpsContact(env, corsHeaders) {
  const row = await getOrCreateContactRow(env);
  return jsonResponse(row, 200, corsHeaders);
}
__name(handleGetOpsContact, "handleGetOpsContact");
async function handlePutOpsContact(request, env, corsHeaders) {
  const body = await safeParseJson(request) ?? {};
  const supabase = getSupabase(env);
  const payload = { ...body, id: 1, updated_at: (/* @__PURE__ */ new Date()).toISOString() };
...
__name(handlePutOpsContact, "handlePutOpsContact");
async function handleGetOpsContactMessages(env, corsHeaders) {
  const supabase = getSupabase(env);
  const { data } = await supabase.from("contact_messages").select("*").order("created_at", { ascending: false });
  return jsonResponse(data ?? [], 200, corsHeaders);
}
__name(handleGetOpsContactMessages, "handleGetOpsContactMessages");
async function handleGetWaitlist(env, corsHeaders) {
  const supabase = getSupabase(env);
  const { data } = await supabase.from("waitlist").select("*").order("created_at", { ascending: false });
  return jsonResponse(data ?? [], 200, corsHeaders);
}
__name(handleGetWaitlist, "handleGetWaitlist");
async function handleGetOpsLogs(request, env, corsHeaders) {
  const url = new URL(request.url);
...
__name(handleGetOpsLogs, "handleGetOpsLogs");
async function logActivity(env, adminId, adminEmail, action, entityType, entityId, metadata, ip) {
  const supabase = getSupabase(env);
...
async function getOrCreateConfigRow(env) {
  const supabase = getSupabase(env);
...
__name(getOrCreateConfigRow, "getOrCreateConfigRow");
async function getOrCreateContactRow(env) {
  const supabase = getSupabase(env);
  const { data, error } = await supabase.from("contact_config").select("*").eq("id", 1).maybeSingle();
  if (data) return data;
  if (error && error.code !== "PGRST116") throw new Error(`DB Error: ${error.message}`);
  const inserted = await supabase.from("contact_config").insert({ id: 1 }).select("*").single();
  return inserted.data;
}
__name(getOrCreateContactRow, "getOrCreateContactRow");
async function findAdminByEmail(supabase, email) {
...
__name(findAdminByEmail, "findAdminByEmail");
async function seedOpsDefaultsIfMissing(env) {
}
__name(seedOpsDefaultsIfMissing, "seedOpsDefaultsIfMissing");
async function handleCreatePresignedLogoUpload(request, env, corsHeaders) {
  return jsonResponse({ upload_url: "https://simulated.com", public_url: "https://cdn.com/logo.png", key: "logo" }, 200, corsHeaders);
}
__name(handleCreatePresignedLogoUpload, "handleCreatePresignedLogoUpload");
async function handleGetPublicPages(env, corsHeaders) {
  return jsonResponse([], 200, corsHeaders);
}
__name(handleGetPublicPages, "handleGetPublicPages");
async function handleGetPublicPageBySlug(env, corsHeaders, slug) {
  return jsonResponse({ message: "Not found" }, 404, corsHeaders);
}
__name(handleGetPublicPageBySlug, "handleGetPublicPageBySlug");
async function handleGetOpsPages(env, corsHeaders) {
  return jsonResponse([], 200, corsHeaders);
}
__name(handleGetOpsPages, "handleGetOpsPages");
async function handleGetOpsPageBySlug(env, corsHeaders, slug) {
  return jsonResponse({ message: "Not found" }, 404, corsHeaders);
}
__name(handleGetOpsPageBySlug, "handleGetOpsPageBySlug");
async function handlePutOpsPageBySlug(request, env, corsHeaders, slug, email) {
  return jsonResponse({}, 200, corsHeaders);
}
__name(handlePutOpsPageBySlug, "handlePutOpsPageBySlug");
async function handleDeleteOpsPageBySlug(env, corsHeaders, slug, email) {
  return jsonResponse({}, 200, corsHeaders);
}
__name(handleDeleteOpsPageBySlug, "handleDeleteOpsPageBySlug");
async function handleGetLegacyContent(env, corsHeaders, slug) {
  return jsonResponse({ slug, body: {}, version: 1 }, 200, corsHeaders);
}
__name(handleGetLegacyContent, "handleGetLegacyContent");
async function handlePutLegacyContent(request, env, corsHeaders, email) {
  return jsonResponse({}, 200, corsHeaders);
}
__name(handlePutLegacyContent, "handlePutLegacyContent");
async function handleGetOpsAdmins(env, corsHeaders) {
  return jsonResponse([], 200, corsHeaders);
}
__name(handleGetOpsAdmins, "handleGetOpsAdmins");
async function handleCreateOpsAdmin(request, env, corsHeaders) {
  return jsonResponse({}, 201, corsHeaders);
}
__name(handleCreateOpsAdmin, "handleCreateOpsAdmin");
async function handleDeleteOpsAdmin(env, corsHeaders, id) {
  return jsonResponse({}, 200, corsHeaders);
}
__name(handleDeleteOpsAdmin, "handleDeleteOpsAdmin");
async function handleGetOpsRoles(env, corsHeaders) {
  return jsonResponse([], 200, corsHeaders);
}
__name(handleGetOpsRoles, "handleGetOpsRoles");
async function handleCreateOpsRole(request, env, corsHeaders) {
  return jsonResponse({}, 201, corsHeaders);
}
__name(handleCreateOpsRole, "handleCreateOpsRole");
async function handleUpdateOpsRole(request, env, corsHeaders, id) {
  return jsonResponse({}, 200, corsHeaders);
}
__name(handleUpdateOpsRole, "handleUpdateOpsRole");
...
async function signToken(c, e) {
  return btoa(JSON.stringify(c)) + "." + btoa(e.OPS_JWT_SECRET || "s");
}
__name(signToken, "signToken");
async function verifyToken(t, e) {
  try {
    return JSON.parse(atob(t.split(".")[0]));
  } catch {
    return null;
  }
}
...
// .wrangler/tmp/bundle-yLaosR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yLaosR/middleware-loader.entry.ts
